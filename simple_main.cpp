...
constexpr wchar_t kPanelClassName[] = L"ModelEquationPanel";
...
std::wstring BuildLoadedText(const ModelData& model, const std::wstring& importSummary) {
    std::wstring text = L"Source: " + model.label
        + L"\r\nVertices: " + std::to_wstring(model.VertexCount())
        + L"\r\nTriangles: " + std::to_wstring(model.TriangleCount());
...
void RefreshStatus() {
    ...
    if (gApp.previewRunning) {
        SetStatusText(L"Building marching cubes preview...");
        return;
    }
...
void MarkDirty() {
    ...
    if (gApp.showingPreview) {
        ...
        ViewportSetModel(gApp.viewport, &gApp.model);
    }
...
void AdoptModel(ModelData&& model) {
    gApp.model = std::move(model);
    gApp.previewModel = {};
    gApp.hasModel = true;
    gApp.fitDirty = true;
    gApp.showingPreview = false;
    ...
    ++gApp.modelRevision;
    ViewportSetModel(gApp.viewport, &gApp.model);
    SetOutputText(BuildLoadedText(gApp.model, gApp.importSummary));
    RefreshStatus();
    UpdateButtons();
}
...
void LoadObjFromDialog(HWND window) {
    ...
    ModelData model;
    std::wstring error;
    if (!LoadObjModel(path, model, error)) {
    ...
    if (kDownscaleEnabled) {
        ...
        std::wstring summary;
        if (!DownsampleModel(model, kDownscaleVertexBudget, downscaled, summary)) {
            MessageBoxW(window, summary.c_str(), L"Downscale failed", MB_OK | MB_ICONERROR);
            return;
        }
        if (downscaled.VertexCount() != model.VertexCount()) {
            model = std::move(downscaled);
            gApp.importSummary = summary;
        } else {
            gApp.importSummary = L"Model stayed at full resolution because it already fit the downscale budget.";
        }
    }
...
void StartMarchingCubes() {
    ...
    RefreshStatus();
    UpdateButtons();
    SetOutputText(L"Building marching cubes preview...");
...
std::filesystem::path BuildReconstructedMeshPath() {
    const std::filesystem::path sourcePath(gApp.model.label);
...
void ExportReconstructedMesh(HWND window) {
    if (!gApp.showingPreview || gApp.previewModel.TriangleCount() == 0) {
        MessageBoxW(window, L"Run Marching Cubes first so there is a reconstructed mesh to export.", L"No reconstructed mesh", MB_OK | MB_ICONINFORMATION);
        return;
    }
...
LRESULT CALLBACK WindowProc(HWND window, UINT message, WPARAM wParam, LPARAM lParam) {
    switch (message) {
        case WM_CREATE: {
            ...
            gApp.exportButton = CreateWindowW(L"BUTTON", L"Export TXT", WS_CHILD | WS_VISIBLE, 0, 0, 0, 0, gApp.settingsPanel, reinterpret_cast<HMENU>(kExportButtonId), nullptr, nullptr);
            gApp.exportMeshButton = CreateWindowW(L"BUTTON", L"Export Reconstructed Mesh", WS_CHILD | WS_VISIBLE, 0, 0, 0, 0, gApp.settingsPanel, reinterpret_cast<HMENU>(kExportMeshButtonId), nullptr, nullptr);
            ...
            SendMessageW(gApp.methodCombo, CB_ADDSTRING, 0, reinterpret_cast<LPARAM>(L"Auto Detect"));
            SendMessageW(gApp.methodCombo, CB_ADDSTRING, 0, reinterpret_cast<LPARAM>(L"Best-Fit Plane"));
            SendMessageW(gApp.methodCombo, CB_ADDSTRING, 0, reinterpret_cast<LPARAM>(L"Best-Fit Sphere"));
            SendMessageW(gApp.methodCombo, CB_ADDSTRING, 0, reinterpret_cast<LPARAM>(L"Best-Fit Cylinder"));
            SendMessageW(gApp.methodCombo, CB_ADDSTRING, 0, reinterpret_cast<LPARAM>(L"Implicit Polynomial (Degree 2)"));
            SendMessageW(gApp.methodCombo, CB_ADDSTRING, 0, reinterpret_cast<LPARAM>(L"Implicit Polynomial (Degree 3)"));
            SendMessageW(gApp.methodCombo, CB_ADDSTRING, 0, reinterpret_cast<LPARAM>(L"RBF Implicit Surface"));
